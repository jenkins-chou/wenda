layui
		...
		.use(
				...
				function() {
					...
					// 监听提交
					form.on('submit(add_know_society)', function(data) {
						parent.resetSwClose(true);
						layer.msg('正在保存。。。', {time : 1000},function(){});
						var url="";
						if(id!=null&&id!='')
							url=ctxPath+"/know_society/updateKownledgeSociety";
						else
							url=ctxPath+"/know_society/addKownledgeSociety";
					...
					//获取信息
					if(id!=null&&id!=''){
						$.getJSON(ctxPath+"/know_society/getKownledgeSocietyById","id="+id,function(jsondata){
