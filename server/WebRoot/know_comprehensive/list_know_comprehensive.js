...
var loadList = function(first) {layui.config({base : "js/"}).use([ 'form', 'layer', 'jquery', 'laypage', 'table' , 'upload'],
					function() {
						...
						table.render({
							elem : '#list',
							url : ctxPath + '/know_comprehensive/getAllKownledgeComprehensive' // 数据接口
							,
							cols : [ [ // 表头
							{
								type : 'checkbox',
						...
						table.on('tool(listtable)', function(obj) {
							...
							if (obj.event === 'del') {
								layer.confirm('你确认删除这条数据吗?', function(index) {
									$.getJSON(ctxPath + "/know_comprehensive/deleteKownledgeComprehensive",
								...
								layer.open({
									...
									move : false,
									content : [
											ctxPath + '/know_comprehensive/toHtmlModify?id='
													+ data.id, 'no' ],
									end : function() {
									}
								});
									...
									layer
											.confirm(
													...
													function(index) {
														$.getJSON(
																		ctxPath
																				+ "/know_comprehensive/deleteKownledgeComprehensiveList",
																		idDatas,
																		function(
																				jsondata) {
																			if (jsondata.code == '200') {
