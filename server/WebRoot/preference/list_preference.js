	...
	layui.config({
			...
			.use([ 'form', 'layer', 'jquery', 'laypage', 'table' ],
					function() {
						...
						table.render({
							elem : '#list',
							url : ctxPath + '/preference/getAllPreference' // 数据接口
							,
							cols : [ [ // 表头
							{
								type : 'checkbox',
						...
						table.on('tool(listtable)', function(obj) {
							...
							if (obj.event === 'del') {
								layer.confirm('你确认删除这条数据吗?', function(index) {
									$.getJSON(ctxPath + "/preference/deletePreference",
								...
								layer.open({
									...
									move : false,
									content : [
											ctxPath + '/preference/toHtmlModify?id='
													+ data.id, 'no' ],
									end : function() {
									}
								});
						...
						var $ = layui.$, active = {
								...
								index = layer.open({
									...
									type : 2,
									area : [ '100%', '100%' ],
									content : ctxPath + "/preference/toHtmlAdd"
								});
							},
									...
									layer
											.confirm(
													...
													function(index) {
														$.getJSON(
																		ctxPath
																				+ "/preference/deletePreferenceList",
																		idDatas,
																		function(
																				jsondata) {
																			if (jsondata.code == '200') {
