layui
		...
		.use(
				...
				function() {
					...
					// 监听提交
					form.on('submit(add_know_mapping)', function(data) {
						parent.resetSwClose(true);
						layer.msg('正在保存。。。', {time : 1000},function(){});
						var url="";
						if(id!=null&&id!='')
							url=ctxPath+"/know_mapping/updateKownledgeMapping";
						else
							url=ctxPath+"/know_mapping/addKownledgeMapping";
					...
					//获取信息
					if(id!=null&&id!=''){
						$.getJSON(ctxPath+"/know_mapping/getKownledgeMappingById","id="+id,function(jsondata){
