	...
	layui.config({
			...
			.use([ 'form', 'layer', 'jquery', 'laypage', 'table' ],
					function() {
						...
						table.render({
							elem : '#list',
							url : ctxPath + '/know_mapping/getAllKownledgeMapping' // 数据接口
							,
							cols : [ [ // 表头
							{
								type : 'checkbox',
						...
						table.on('tool(listtable)', function(obj) {
							...
							if (obj.event === 'del') {
								layer.confirm('你确认删除这条数据吗?', function(index) {
									$.getJSON(ctxPath + "/know_mapping/deleteKownledgeMapping",
								...
								layer.open({
									...
									move : false,
									content : [
											ctxPath + '/know_mapping/toHtmlModify?id='
													+ data.id, 'no' ],
									end : function() {
									}
								});
						...
						var $ = layui.$, active = {
								...
								index = layer.open({
									...
									type : 2,
									area : [ '100%', '100%' ],
									content : ctxPath + "/know_mapping/toHtmlAdd"
								});
							},
									...
									layer
											.confirm(
													...
													function(index) {
														$.getJSON(
																		ctxPath
																				+ "/know_mapping/deleteKownledgeMappingList",
																		idDatas,
																		function(
																				jsondata) {
																			if (jsondata.code == '200') {
