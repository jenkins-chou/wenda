	...
	layui.config({base : "js/"}).use([ 'form', 'layer', 'jquery', 'laypage', 'table', 'upload' ],
					function() {
						...
						table.on('tool(listtable)', function(obj) {
							...
							} else if (obj.event == 'pb') {
								$.getJSON(ctxPath + "/user/update", idDatas
										+ "&&state=屏蔽", function(jsondata) {
									if (jsondata.code == '200') {
										layer.msg('成功', {
											time : 1000,
											shade : [ 0.001, '#ffffff' ]
							...
							} else if (obj.event == 'jcpb') {
								$.getJSON(ctxPath + "/user/update", idDatas
										+ "&&state=正常", function(jsondata) {
									if (jsondata.code == '200') {
										layer.msg('成功', {
											time : 1000,
											shade : [ 0.001, '#ffffff' ]
...
//	
//	
// $.get(ctxPath+"/user/list", function(data){
// newsData=data;
// //执行加载数据的方法
// newsList();
// })
