layui
		...
		.use(
				...
				function() {
					...
					// 监听提交
					form.on('submit(add_know_natural)', function(data) {
						parent.resetSwClose(true);
						layer.msg('正在保存。。。', {time : 1000},function(){});
						var url="";
						if(id!=null&&id!='')
							url=ctxPath+"/know_natural/updateKownledgeNatural";
						else
							url=ctxPath+"/know_natural/addKownledgeNatural";
					...
					//获取信息
					if(id!=null&&id!=''){
						$.getJSON(ctxPath+"/know_natural/getKownledgeNaturalById","id="+id,function(jsondata){
